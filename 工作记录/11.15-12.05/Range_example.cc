...
#include "llvm/Analysis/CFG.h"//chn add 20211020
#include <llvm/IR/MDBuilder.h>//chn add 20211020
...
CRange RangePass::getRange(BasicBlock *BB, Value *V)
{
	...
	if (CallInst *CI = dyn_cast<CallInst>(V)) {
		...
		/*causion: load store without annotations, neither in VRM */
...
bool RangePass::updateRangeFor(Instruction *I)
{
	...
	{
		dumpRange();
		dbgs()<<"none of Store Return or Call ...\n";
		//dumpVRM(I);
		//dbgs()<<"\n\n";
	}
	#endif
								...
								ValueRangeMap &VRM)
{
	...
	//chn debug 2021-11-17
	//dbgs()<<"ICmpInst: "<<*ICI<<"\n";
	dbgs()<<"          LHS: "<<*LHS<<",  LCR: "<<LCR<<"\n";
	dbgs()<<"                LCR.getLower(): "<<LCR.getLower()<<"\n";
	dbgs()<<"                LCR.getUpper(): "<<LCR.getUpper()<<"\n";
	dbgs()<<"          RHS: "<<*RHS<<",  RCR:"<<RCR<<"\n";
	dbgs()<<"                RCR.getLower(): "<<RCR.getLower()<<"\n";
	dbgs()<<"                RCR.getUpper(): "<<RCR.getUpper()<<"\n";
	RCR.match(LCR);
	...
	if (BI->getSuccessor(0) == BB) {
		...
		//VRM.insert(std::make_pair(RHS, RCR.intersectWith(PLCR)));//chn 2021-11-17:LCR. ->RCR.
		dbgs()<<"          true target..... "<<"\n";//chn debug 2021-11-17
	} else {
		// false target, use inverse predicate
		// N.B. why there's no getSwappedInversePredicate()...
		ICI->swapOperands();
		PLCR = CRange::makeICmpRegion(
									ICI->getInversePredicate(), LCR);//chn 2021-11-18:RCR ->LCR
		ICI->swapOperands();
		PRCR = CRange::makeICmpRegion(
									ICI->getInversePredicate(), RCR);
		//VRM.insert(std::make_pair(LHS, LCR.intersectWith(PRCR)));
		//VRM.insert(std::make_pair(RHS, RCR.intersectWith(PLCR)));//chn 2021-11-18:LCR. ->RCR.
		//chn debug 2021-11-17
		dbgs()<<"          false target..... "<<"\n";
	}
	
	/*update range of LHS and RHS, constrain LCR, i.e. intersect or replace, but not Union with the existing Range.*/
	/*however, insert cann't work if VRM already contains key "LHS" or "RHS" */
	/*array element assignment can tackle this problem as it can cover existing Range */
	//VRM.insert(std::make_pair(LHS, LCR.intersectWith(PRCR)));
	//VRM.insert(std::make_pair(RHS, RCR.intersectWith(PLCR)));
	if (VRM.find(LHS) != VRM.end())
		VRM.erase(LHS);
	if (VRM.find(RHS) != VRM.end())
		VRM.erase(RHS);
	...
	dbgs()<<"                PLCR: "<<PLCR<<"\n";
	dbgs()<<"                PRCR: "<<PRCR<<"\n";	
	dbgs()<<"                LCR.intersectWith(PRCR): "<<LCR.intersectWith(PRCR)<<"\n";
	dbgs()<<"                RCR.intersectWith(PLCR): "<<RCR.intersectWith(PLCR)<<"\n\n";
	dbgs()<<"                LCR.intersectWith(PRCR).getUpper(): "<<LCR.intersectWith(PRCR).getUpper()<<"\n";
	dbgs()<<"                LCR.intersectWith(PRCR).getLower(): "<<LCR.intersectWith(PRCR).getLower()<<"\n";
	//dbgs()<<"                RCR.intersectWith(PLCR): "<<RCR.intersectWith(PLCR)<<"\n\n";
...
bool RangePass::doFinalization(Module *M) {
	...
	for (Module::iterator f = M->begin(), fe = M->end(); f != fe; ++f) {
		...
		for (inst_iterator i = inst_begin(F), e = inst_end(F); i != e; ++i) {
			...
			//Value *RL[] = { Lo, Hi };
			MDBuilder MDB(VMCtx);
			MDNode *MD = MDNode::get(VMCtx,{MDB.createConstant(Lo), MDB.createConstant(Hi)});
...
/*chn add for debug 2021-11-12*/
void RangePass::dumpVRM()
{
	raw_ostream &OS = dbgs();
	for(FuncValueRangeMaps::iterator iFVRM = FuncVRMs.begin(),eFVRM = FuncVRMs.end();iFVRM!=eFVRM;++iFVRM)
	{
		OS<<"BB: "<<iFVRM->first->getName()<<"\n";
		ValueRangeMap VRM = iFVRM->second;
		for (ValueRangeMap::iterator i = VRM.begin(), e = VRM.end(); i != e; ++i) {
			OS <<"      Value: "<< *i->first << "\n ";
			OS <<"              "<< i->second << "\n";
		}
		OS<<"\n";
...
/*chn add for debug 2021-11-15*/
void RangePass::dumpVRM(Instruction *I)
{
	raw_ostream &OS = dbgs();
	ValueRangeMap VRM = FuncVRMs[I->getParent()];
	OS<<"BB: "<<I->getParent()->getName()<<"\n";
	for (ValueRangeMap::iterator i = VRM.begin(), e = VRM.end(); i != e; ++i) {
			OS <<"      Value: "<< *i->first << "\n ";
			OS <<"              "<< i->second << "\n";
		}
...
/*chn add for debug 2021-11-17*/
void RangePass::dumpVRM(BasicBlock *BB)
{
	raw_ostream &OS = dbgs();
	ValueRangeMap VRM = FuncVRMs[BB];
	OS<<"BB: "<<BB->getName()<<"\n";
...
/*chn add for debug 2021-11-17*/
void RangePass::dumpVRM(ValueRangeMap *VRM)
{
	raw_ostream &OS = dbgs();
	for (ValueRangeMap::iterator i = VRM->begin(), e = VRM->end(); i != e; ++i) {
			OS <<"      Value: "<< *i->first << "\n ";
